...
{
	private Dictionary<IntPtr, VaalSoulStructure> ActiveSkillsVaalSouls { get; } = new Dictionary<IntPtr, VaalSoulStructure>();
	...
	internal override void ToImGui()
	{
		...
		if (ImGui.TreeNode("Active Skills"))
		{
			foreach (KeyValuePair<string, ActiveSkillDetails> activeSkill in ActiveSkills)
			{
				...
				(obj, skilldetails) = (KeyValuePair<string, ActiveSkillDetails>)(ref activeSkill);
				if (obj == null)
				{
					obj = "";
				}
	...
	protected override void UpdateData(bool hasAddressChanged)
	{
		...
		{
			ActiveSkillCooldowns[cooldowns[k].UnknownIdAndEquipmentInfo] = cooldowns[k];
		}
		...
		for (int j = 0; j < activeSkills.Length; j++)
		{
			...
			{
				ref IntPtr activeSkillsDatPtr = ref skillDetails.ActiveSkillsDatPtr;
				string name;
				(name, activeSkillsDatPtr) = ((string, IntPtr))Core.GgpkObjectCache.AddOrGetExisting(skillDetails.GrantedEffectsPerLevelDatRow, (IntPtr key) => (reader.ReadUnicodeString(reader.ReadMemory<IntPtr>(reader.ReadMemory<IntPtr>(key))), reader.ReadMemory<GrantedEffectsDatOffset>(reader.ReadMemory<GrantedEffectsPerLevelDatOffset>(key).GrantedEffectDatPtr).ActiveSkillDatPtr));
