...
public class Buffs : ComponentBase
{
	...
	protected override void UpdateData(bool hasAddressChanged)
	{
		...
		StatusEffects.Clear();
		IntPtr[] statusEffects = reader.ReadStdVector<IntPtr>(data.StatusEffectPtr);
		...
		for (int i = 0; i < statusEffects.Length; i++)
		{
			...
			if (statusEffectData.BuffDefinationPtr != IntPtr.Zero)
			{
				...
				(string, byte) obj = ((string, byte))Core.GgpkObjectCache.AddOrGetExisting(statusEffectData.BuffDefinationPtr, (IntPtr key) => GetNameFromBuffDefination(key));
