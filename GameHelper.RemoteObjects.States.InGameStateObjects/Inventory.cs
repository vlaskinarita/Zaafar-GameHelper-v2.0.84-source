...
public class Inventory : RemoteObjectBase
{
	...
	public ConcurrentDictionary<IntPtr, Item> Items { get; } = new ConcurrentDictionary<IntPtr, Item>();
	...
	public Item this[int y, int x]
	{
		get
		{
			...
			IntPtr itemAddr = itemsToInventorySlotMapping[index];
	...
	internal override void ToImGui()
	{
		...
		foreach (KeyValuePair<IntPtr, Item> item in Items)
		{
	...
	protected override void UpdateData(bool hasAddressChanged)
	{
		...
		itemsToInventorySlotMapping = reader.ReadStdVector<IntPtr>(invInfo.ItemList);
		if (hasAddressChanged)
		{
			Items.Clear();
		}
		foreach (KeyValuePair<IntPtr, Item> item3 in Items)
		{
			item3.Value.IsValid = false;
		}
		Parallel.ForEach(itemsToInventorySlotMapping.Distinct(), delegate(IntPtr invItemPtr)
		{
