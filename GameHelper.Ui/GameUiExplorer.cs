					...
					if (ImGui.CollapsingHeader(eleName + $"##{i}", ref isRequired, ImGuiTreeNodeFlags.DefaultOpen))
					{
						...
						if (ImGui.BeginCombo($"Children##{i}", current.CurrentChildPreview))
						{
							...
							for (int k = 0; k < current.Children.Count; k++)
							{
								...
								if (ImGui.Selectable($"{k}-{child2.Address.ToInt64():X}-{child2.Id}", selected))
								{
									current.CurrentChildIndex = k;
									current.CurrentChildPreview = $"{child2.Address.ToInt64():X}-{child2.Id}";
									Elements[i] = current;
								}
								if (child2.IsVisible)
								{
								...
								if (ImGui.IsItemHovered())
								{
									ImGuiHelper.DrawRect(child2.Postion, child2.Size, byte.MaxValue, 64, 64);
								}
							}
							ImGui.EndCombo();
						}
						if (ImGui.IsItemHovered())
						{
							for (int j = 0; j < current.Children.Count; j++)
							{
								...
								ImGuiHelper.DrawRect(child.Postion, child.Size, byte.MaxValue, 64, 64);
							}
						}
						...
						ImGui.SameLine();
						if (current.Element.ParentAddress != IntPtr.Zero)
						{
							if (ImGui.Button($"Go to parent##{i}"))
							{
								Elements[i] = CreateUiElement(Core.States.InGameStateObject.GameUi.Parents.GetParent(current.Element.ParentAddress));
							}
						}
						else
						{
