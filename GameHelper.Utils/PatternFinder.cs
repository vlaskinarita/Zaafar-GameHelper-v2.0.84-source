...
internal static class PatternFinder
{
	...
	internal static Dictionary<string, int> Find(SafeMemoryHandle handle, IntPtr baseAddress, int processSize)
	{
		...
		Parallel.For(0, totalReadOperations, pOptions, delegate(int i, ParallelLoopState state1)
		{
			...
			{
				byte[] processData = handle.ReadMemoryArray<byte>((nint)baseAddress + currentOffset, nsize);
