...
public class GameProcess
{
	...
	internal Dictionary<string, IntPtr> StaticAddresses { get; } = new Dictionary<string, IntPtr>();
	...
	{
		while (!Information.HasExited && Information.MainWindowHandle.ToInt64() > 0 && !closeForcefully)
		{
			UpdateIsForeground();
	...
	private IEnumerator<Wait> FindStaticAddresses()
	{
		while (true)
		{
			...
			if (baseAddress == (nint)IntPtr.Zero)
			{
				continue;
			}
